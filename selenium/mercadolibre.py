...
try:  # Encerramos todo en un try catch para que si no aparece el discilamer, no se caiga el codigo
    disclaimer = driver.find_element(
        By.XPATH, '//button[@data-testid="action:understood-button"]')
...
while PAGINACION_MAX > PAGINACION_ACTUAL:
    ...
    links_productos = driver.find_elements(
        By.XPATH, '//a[@class="ui-search-item__group__element shops__items-group-details ui-search-link"]')
    links_de_la_pagina = []
    ...
    for link in links_de_la_pagina:
        ...
        try:
            ...
            precio = driver.find_element(
                By.XPATH, '//span[contains(@class,"ui-pdp-price")]').text
            print(titulo)
            print(precio.replace('\n', '').replace('\t', ''))

            # click en el boton de retroceso
            driver.back()
        except Exception as e:
            print(e)
    ...
    try:
        ...
        puedo_seguir_horizontal = driver.find_element(
            By.XPATH, '//span[text()="Siguiente"]')
